	...
	public static void executar(int interval, boolean display) {
		try {
                if (scheduler1 == null || scheduler1.isShutdown()) {
                    JobDetail job1 = JobBuilder.newJob(JobNFCe.class)
...
//                    JOptionPane.showMessageDialog(null, "<html><h1 style='font-family: Calibri; font-size: 18pt; color: red;'>O Monitoramento da Importação Automática já está Ativado! . . .</html>");
        ...
        public static void finalizar(boolean display) {
            try {
                String msg = "";
                //JOptionPane.showMessageDialog(null, "estado do job: " + scheduler1.isShutdown());
                if (scheduler1 != null && !scheduler1.isShutdown()) {
                    scheduler1.shutdown();
                    msg = "<html><h1 style='font-family: Calibri; font-size: 18pt; color: blue;'>Finalizou a espera do arquivo XML para Gerar a NFCe . . .</html>";
                } else {
                ...
                Logger.getLogger(CronImportNFCe.class.getName()).log(Level.SEVERE, null, ex);
            }
            
        }
	public static void main(String[] args) {
           CronImportNFCe ciNFCe = new CronImportNFCe(); 
           ciNFCe.executar(5, true);
        }
}